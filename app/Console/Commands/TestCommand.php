...
class TestCommand extends Command
{
    ...
    public function handle()
    {
        ...
        for ($i = 1; $i <= 6; $i++) {
            $url = "https://www.etsy.com/shop/ccanmistore?section_id=1&page=$i";
            $response = Http::get($url);
            $html = $response->body();

            $crawler = new Crawler($html);
            $products= $crawler->filter('.listing-link.wt-display-inline-block.wt-transparent-card')->each(function (Crawler $node) {
                $href = $node->filter('a')->attr('href');
                $id = $node->filter('a')->attr('data-listing-id');
                $image = $node->filter('img')->attr('src');
                $title = $node->filter('.wt-text-caption.v2-listing-card__title.wt-text-truncate')->text();
                return [
                    'id' => $id,
                    'href' =>$href,
                    'image' => $image,
        ...
        foreach ($products as &$product) {
            $product['image'] = str_replace('il_340x270', 'il_540x360', $product['image']);
        }
