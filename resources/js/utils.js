...
const dataSourceMap = window.$json.dataSourceMap || {};
export async function getDataSource(datasource, field) {
    ...
    const res = await $get('/xadmin/data-source/index', params, false);

    dataSourceMap[datasource] = res.data;
    return dataSourceMap[datasource];
}

export async function getDataSourceAsync(datasource, field) {
    const res = await $get('/xadmin/data-source/index', {
        datasource,
        field: field.field,
        value: field.value
    }, false);
