<template>
    <div class="container-fluid" >
        <ActionBar type="index"
                   createUrl="/xadmin/allocation_content_units/create"
                   title="Allocation_content_unitIndex"/>
        <div class="row">
            <div class="col-lg-12">
                        ...
                        <table class=" table  table-head-custom table-head-bg table-vertical-center">
                            ...
                            <tr v-for="entry in entries" @click="edit(entry.id, $event)">
                                <td v-text="entry.id"></td>
                                                                    ...
                                                                    <td v-text="entry.course_id"></td>
                                                                    <td v-text="entry.course_component_id"></td>
                                
                                <td class="">
<!--                                    <a :href="'/xadmin/allocation_content_units/edit?id='+entry.id" style="margin-right: 10px"><i style="font-size:1.3rem" class="fa fa-edit"></i></a>-->
...
<script>
    ...
    export default {
        name: "Allocation_content_unitsIndex.vue",
        ...
        methods: {
            edit: function (id, event){
                if (!$(event.target).hasClass('deleted')){
                    window.location.href='/xadmin/allocation_content_units/edit?id='+ id;
                }

            },
            async load() {
                let query = $router.getQuery();
                const res  = await $get('/xadmin/allocation_content_units/data', query);
            ...
            async remove(entry) {
                ...
                const res = await $post('/xadmin/allocation_content_units/remove', {id: entry.id});
            ...
            async toggleStatus(entry) {
                const res = await $post('/xadmin/allocation_content_units/toggleStatus', {
                    id: entry.id,
                    status: entry.status
                });
