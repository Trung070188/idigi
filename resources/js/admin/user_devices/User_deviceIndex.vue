                ...
                <div class="modal-content box-shadow-main paymment-status" style="margin-right:20px; left:140px">
                    ...
                    <div class="content">
                        ...
                        <input type="text" class="form-control " placeholder="Device name" aria-label=""
                               style="margin-bottom: 10px" aria-describedby="basic-addon1" v-model="entry.device_name">
                        <error-label for="f_category_id" :errors="errors.device_name"></error-label>

                        <input type="text" class="form-control col-2" placeholder="Enter the device information code"
                               aria-label="" aria-describedby="basic-addon1" v-model="entry.device_uid">
            ...
            <div class="modal-dialog modal-dialog-centered popup-main-1" role="document">
                <div class="modal-content">
                    ...
                    <div class="content">
                        ...
                        <div id="token" v-if="token" ref="token"
                             style="font-size: 16px; word-wrap: break-word;margin-top:10px;word-break: normal;">
                            {{ token }}
                        </div>
                    </div>
        ...
        <div class="modal fade" style="margin-right:50px;border:2px solid #333333  " id="deviceConfirmLimit"
            ...
            <div class="modal-dialog modal-dialog-centered popup-main-1" role="document"
                ...
                <div class="modal-content box-shadow-main paymment-status"
                     style="margin-right:20px; left:140px;margin-bottom: 200px;padding: 20px">
                    <div class="close-popup" data-dismiss="modal"></div>
                    <h3 class="popup-title success" style="margin-left:100px">Can not add more devices</h3>
                    <div class="content">
                        <p>Bạn chỉ được truy cập vào tối đa 3 thiết bị, hãy xóa bớt thiết bị cũ nếu muốn truy cập vào
                            thiết bị mới.</p>
                    </div>
                </div>
            </div>
                ...
                <div class="card card-custom card-stretch gutter-b">
                    <div class="card-body d-flex flex-column" style="height: 563px">
                        <div v-for="entry in entries" class="row" v-if="entry.id==auth.id">
                            <button v-if="entry.user_devices.length<3 " type="button"
                                    class="col-lg-2 btn btn-danger modal-devices " @click="modalDevice()">
                                Add more device
                            </button>
                            <button v-if="entry.user_devices.length>=3" type="button"
                                    class="col-lg-2 btn btn-danger modal-devices " @click="closeModal()">
                                Add more device
                            </button>
                        </div>
                        <div class="row width-full" v-for="entry in entries" v-if="entry.id==auth.id">
                            <div class="col-lg-12 body " v-for="device in entry.user_devices">
                                ...
                                <div class="form-group mx-sm-3 mb-2"
                                     style="position: absolute;right:65px;margin-top: -33px;">
                                    <button type="button"
                                            class="btn btn-flex btn-light  fw-verify " style="margin-right: 5px"
                                            @click="editModalDevice(device.id,device.device_name,device.secret_key)">
                                        Get activity code
                                    </button>
                                    <button type="button"
                                            class="btn btn-flex btn-light  fw-bolder " @click="remove(device)">Delete
                                        device
                                    </button>
                                </div>
                            </div>
